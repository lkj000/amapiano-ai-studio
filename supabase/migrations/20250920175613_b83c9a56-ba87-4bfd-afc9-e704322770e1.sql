...
ON public.partnership_requests 
FOR SELECT 
USING (requester_id = auth.uid() OR auth.uid() IN (
  SELECT user_id FROM public.profiles WHERE is_seller = true
));

CREATE POLICY "Administrators can manage all requests" 
ON public.partnership_requests 
FOR ALL 
USING (auth.uid() IN (
  SELECT user_id FROM public.profiles WHERE is_seller = true
...
CREATE POLICY "System can manage gap analysis" 
ON public.content_gap_analysis 
FOR ALL 
USING (true);
...
-- Insert sample content gap data
